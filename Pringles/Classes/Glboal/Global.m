...
-(void) setLocationConfirm:(NSString *)loc{
	...
	if (lblConfirm) {
		...
		lblConfirm.textAlignment = UITextAlignmentCenter;
...
+ (void)messagetoast:(NSString*)string target:(UIViewController*)pt{
	...
	BaseController *baseCon = (BaseController*)pt;
	frame.origin.x = (pt.view.frame.size.width - [lblToast getWidth])/2;
	frame.origin.y = (460 - 80 - baseCon.tabBarHeight - [lblToast getHeigth]);
...
/**
 post请求 解析json数据
 */
-(NSDictionary*) PostPaserHelper:(id<GlobalDelegate>)taget :(NSString*)url :(NSArray*)params :(ProcessType)processType
{
	//NSLog(@"%@",url);
	...
	myRequest.globalDelegate = taget;
...
-(NSDictionary*) PostDataHeler:(id<GlobalDelegate>)taget :(NSString*)url :(NSArray*)params :(NSData*)imageData :(ProcessType)processType
{
	ASIFormDataRequest *myRequest = [[[ASIFormDataRequest alloc] initWithURL:[NSURL URLWithString:url]] autorelease];	
	myRequest.processType = processType;
	//[myRequest addRequestHeader:@"Accept" value:@"application/xml"];//add header	
	...
	[myRequest setPostValue:@"iphone" forKey:@"apptype"];
	if(imageData)
		[myRequest setData:imageData forKey:@"imgData"];
...
-(NSDictionary*) GetPaserHelper:(id<GlobalDelegate>)taget :(NSString*)url :(ProcessType)processType
{
	//UTF8 to gb2312
	...
	url = [url stringByAppendingFormat:@"&webname=%@&token=%@&apptype=iphone&appuuid=%@",[LoginResult GetInstance].webname,[LoginResult GetInstance].token,[Global encodedUrlString:[Config GetInstance].firstTime]];
	ASIHTTPRequest *myRequest = [[[ASIHTTPRequest alloc] initWithURL:[NSURL URLWithString:url]] autorelease];
	myRequest.globalDelegate = taget;
	myRequest.processType = processType;
	myRequest.delegate = self;
	[myRequest setDidStartSelector:@selector(requestGetDidStart:)];
	[myRequest setDidFailSelector:@selector(requestGetDidFailed:)];
	[myRequest setDidFinishSelector:@selector(requestGetDidSuccess:)];
...
-(NSArray*) GetPaserHelperArray:(id<GlobalDelegate>)taget :(NSString*)url :(ProcessType)processType
{
	//UTF8 to gb2312
	//myURL = [myURL stringByAddingPercentEscapesUsingEncoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000)];
	url = [url stringByAppendingFormat:@"&webname=%@&token=%@&apptype=iphone",[LoginResult GetInstance].webname,[LoginResult GetInstance].token];
	ASIHTTPRequest *myRequest = [[[ASIHTTPRequest alloc] initWithURL:[NSURL URLWithString:url]] autorelease];
	myRequest.globalDelegate = taget;
	myRequest.processType = processType;
	myRequest.delegate = self;
    ...
    NSArray *dir = [myResponseStr JSONValue];
	[myResponseStr release];
	return dir;
}
